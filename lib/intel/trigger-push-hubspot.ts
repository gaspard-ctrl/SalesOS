...
export async function triggerPushHubspot(listId: string, userId: string, origin: string): Promise<void> {
  const isNetlifyEnv = !!(process.env.NETLIFY || process.env.URL || process.env.DEPLOY_URL);

  if (!isNetlifyEnv) {
    void pushListToHubspot(listId, userId).catch((e) =>
      console.error(
  ...
  try {
    const res = await fetch(triggerUrl, {
      ...
      body: JSON.stringify({ id: listId, userId }),
