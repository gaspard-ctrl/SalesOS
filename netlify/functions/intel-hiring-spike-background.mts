...
import { runHiringSpikeAgent } from "../../lib/intel/agents/hiring-spike";
import { logAgentRun } from "../../lib/intel/log-agent-run";
import { db } from "../../lib/db";

// Background function : 30 sociétés Radar x getCompanyJobs Netrows + filtre
// ICP RH/L&D, budget 15 min vs ~26s sync.
//
...
export default async (req: Request, _ctx: Context) => {
  ...
  try {
    const result = await runHiringSpikeAgent();
    signalsCount = result.signalsCount;
    payload = result;
  } catch (e) {
    ok = false;
    errorText = e instanceof Error ? e.message : String(e);
    console.error("[intel-hiring-spike-background] failed:", e);
  }
  ...
  if (userId) {
    ...
    if (upsertErr) {
      console.error("[intel-hiring-spike-background] intel_agent_runs upsert failed:", upsertErr);
    }
  }

  await logAgentRun({
    agentId: "hiring-spike",
    triggeredBy,
    userId,
    startedAt,
    status: ok ? "ok" : "error",
