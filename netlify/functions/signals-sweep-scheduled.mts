 ...
 * (sweep complet watchlist + discovery, avec Apollo et datasets LinkedIn). On
 * découple pour ne pas tenir la durée d'un scheduled function classique : le sweep
 * enchaîne beaucoup d'appels Claude + scraping, c'est un job long.
 */
export default async () => {
  ...
  try {
    const res = await fetch(`${siteUrl}/.netlify/functions/signals-sweep-background`, {
      ...
      body: JSON.stringify({ feed: "both", includeApollo: true, includeSlowSources: true }),
