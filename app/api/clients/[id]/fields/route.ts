...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!sectionKey || !fieldKey) {
    return NextResponse.json({ error: "sectionKey and fieldKey required" }, { status: 400 });
  }
  ...
  if (!section) return NextResponse.json({ error: `unknown section: ${sectionKey}` }, { status: 400 });
  const fieldDef = section.fields.find((f) => f.key === fieldKey);
  if (!fieldDef) return NextResponse.json({ error: `unknown field: ${fieldKey}` }, { status: 400 });
