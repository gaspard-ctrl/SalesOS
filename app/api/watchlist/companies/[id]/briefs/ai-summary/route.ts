...
export interface RefreshAiSummaryResponse {
  ok: boolean;
  queued?: boolean;
...
const BG_FN = "watchlist-ai-summary-background";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (process.env.NETLIFY === "true" && cronSecret) {
    fetch(`${siteUrl}/.netlify/functions/${BG_FN}`, {
      ...
      console.error(`[briefs/ai-summary] background invoke failed:`, e);
    });
    return NextResponse.json({ ok: true, queued: true, briefId }, { status: 202 });
  }

  after(async () => {
    const res = await runAiSummary({ scopeCompanyId: id, userId: user.id });
    if (!res.ok) {
      console.error("[briefs/ai-summary] dev run failed:", res.error);
    }
  });
